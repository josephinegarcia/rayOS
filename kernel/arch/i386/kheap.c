...
uint32_t initialBase = 0x0;
uint32_t currentBase = 0x0;
uint32_t maxBase = 0x0;
...
int kmalloc_early_init_grub(struct multiboot_info * mbi)
{
  ...
  {
    uint32_t * mem_info_ptr = (uint32_t *)mbi->mmap_addr; 

    while(mem_info_ptr < (uint32_t *)mbi->mmap_addr+mbi->mmap_length)
    {
...
{
  if(!initialized)
    printf("kmalloc_early: no base address initialized\n");

  if(!size)
    return NULL;

  if(size+currentBase > maxBase){}
    printf("kmalloc_early: %u byte allocation caused us to run out of memory (%u bytes left)\n", size, maxBase - currentBase);
  

  uint32_t addr = currentBase;
...
{
  if(!initialized)
    printf("kmalloc_early_align: no base address initialized\n");
